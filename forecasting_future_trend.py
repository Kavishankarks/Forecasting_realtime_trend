...
def multiKeywordGeneration():
	...
	if(st.button("Get related queries of "+q_rel[0],help="Click on the button to get all the queries related to"+q_rel[0])):
		...
		st.title("\nKeys after removing stop words:")
		for i in q_rel[0].split()[:-1]:
			rel_topics.insert(0,i)
		# rel_topics.insert(0,q_rel[0].split())
		# st.write(*rel_topics)
...
def keyword():
	...
	class thread_link1:
	    ...
	    def __init__(self,i):
	        self.i=i
	        t=threading.Thread(target=self.link1)
	        t.start()
	...
	class thread_link2:
	    ...
	    def link2(self):
	        try:
	            dict={}
	            article = Article(df['link'][self.ind],config=config)
	            article.download()
	            article.parse()
	            article.nlp()
	            dict['Date']=df['date'][self.ind]
	            dict['Media']=df['media'][self.ind]
	...
	#             list.append(dict)
	    ...
	    def __init__(self,ind):
	        self.ind=ind
	        t=threading.Thread(target=self.link2)
	        t.start()
	...
	print("Articles Link Extracted Successfully ")
	...
	class thread:
	    ...
	    def function(self):
	        try:
	            article = Article(self.aa)
	            article.download()
	            article.parse()
	#             print(article.text)
	#             print("----------------")
	            rr=self.n+str(thread.o)+'.txt'
	            file = open(rr,'a')
	...
	name=n+str(i)+".txt"
	print("reading File1:")
	data=open(name,"r", encoding="utf8")
	text1=data.read()
	...
	data2=open(name,"r", encoding="utf8")
	text2=data2.read()
	...
	data3=open(name,"r", encoding="utf8")
	text3=data3.read()
	...
	data4=open(name,"r", encoding="utf8")
	text4=data4.read()
	...
	numOfKeywords = 80
	print("Parameters of custom Keyword extractor")
	print("Num of keywords:",numOfKeywords,"\nDuplication threshold:",deduplication_threshold)
	...
	import threading
	def func(text):
	    k=custom_kw_extractor.extract_keywords(text)
	    Global.append(k)
	t1 = threading.Thread(target=func, args=(text1,))
	t2 = threading.Thread(target=func, args=(text2,))
	t3 = threading.Thread(target=func, args=(text3,))
	t4 = threading.Thread(target=func, args=(text4,))
	t1.start()
	t2.start()
	t3.start()
	t4.start()
	...
	i=0
	kwlist.sort()
	print("\n\n \t\tKeywords extracted for the Given Keyword are:\n\n")
	while i<n-1:
	    print(kwlist[i],"\t\t",kwlist[i+1])
	    i=i+2
...
def main():
    ...
    elif page == "Multivariate  approach":
